...
interface NavItemProps
  extends LinkProps<object>, HTMLAttributes<HTMLAnchorElement> {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? "bg-light" : "hover:bg-active hover:text-text-primary",
      )}
    >
      <div
        className={cn(
          ...
          isActive
            ? "text-text-primary"
            : "text-text-sub group-hover:text-text-primary",
        )}
      >
        <Image src={icon as StaticImport} alt={label} className="w-5 h-5" />
      </div>
      <span
        className={`font-medium ${isActive ? "text-strong950" : "text-sub600"}`}
      >
        {label}
      </span>
      {badge && (
  ...
  return (
    <aside className="bg-transparent border-active flex flex-col h-full">
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
        <NavItem href="/" icon={BrowseIcon} label="Browse Cases" />

        <NavItem href="/my-cases" icon={FolderPaperIcon} label="My Cases" />
